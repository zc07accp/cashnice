...
static inline NSString* PELocalizedString(NSString* key, NSString* comment)
{
    ...
    if (isEng) {
        ...
        if ([[key lowercaseString] isEqualToString:@"cancel"]) {
            return @"取消";
        }
        else if ([[key lowercaseString] isEqualToString:@"done"]) {
            return @"完成";
        }
        else {
            return key;
...
//    self.navigationController.toolbar.translucent = NO;
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:PELocalizedString(@"Cancel", @"") style:UIBarButtonItemStylePlain target:self action:@selector(cancel:)];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:PELocalizedString(@"Done", @"") style:UIBarButtonItemStylePlain target:self action:@selector(done:)];
