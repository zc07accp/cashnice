...
- (void)setKeepingAspectRatio:(BOOL)keepingAspectRatio
{
    ...
    if (self.keepingAspectRatio) {
        CGFloat width = CGRectGetWidth(self.bounds);
        CGFloat height = CGRectGetHeight(self.bounds);
        //self.fixedAspectRatio = 2.0;//fminf(width / height, height / width);
...
- (CGRect)cropRectMakeWithResizeControlView:(PEResizeControl*)resizeControlView
{
    ...
    //[Util logRect:rect tag:@"rect"];
    //[Util logRect:self.initialRect tag:@"intial"];
    ...
    CGFloat minHeight = minWidth / self.fixedAspectRatio;
    //L2(@"min - w: %f h: %f", minWidth, minHeight);
