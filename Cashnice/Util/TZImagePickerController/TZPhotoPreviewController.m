...
- (void)configCustomNaviBar {
    ...
    if (self.photoPreviewType == PhotoPreviewTypeDelete) {
        [_selectButton setImage:[UIImage imageNamed:@"iou_photo_delete.png"] forState:UIControlStateNormal];
        [_selectButton setImage:[UIImage imageNamed:@"iou_photo_delete.png"] forState:UIControlStateSelected];
    }else{
    ...
    [_titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_naviBar);
        make.bottom.equalTo(_naviBar);
        make.left.equalTo(_backButton.mas_right);
        make.right.equalTo(_selectButton.mas_left);
    }];
    
    _selectButton.hidden = self.photoPreviewType == PhotoPreviewTypeNone;
    
    [self.view addSubview:_naviBar];
}

- (void)configBottomToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 50, self.view.tz_width, 50)];
    
    _toolBar.backgroundColor = ZCOLOR(@"#c5c6c7");
    _toolBar.alpha = 0.7;
    ...
    NSString *buttonTitle = [NSString stringWithFormat:@"完成(%zd/%zd)",_tzImagePickerVc.selectedModels.count, _tzImagePickerVc.maxImagesCount];
    ...
    _toolBar.hidden = self.photoPreviewType != PhotoPreviewTypeOperation;
...
- (NSMutableArray *)previewViews{
    if (! _previewViews) {
        ...
        for (int i = 0; i <= tzImagePickerVc.maxImagesCount; i++) {
            ...
            [_toolBar addSubview:previewView];
...
- (void)buttonpress1:(UIButton *)button {
    ...
    if (tzImagePickerVc.selectedModels.count > index) {
        ...
        if ([[[TZImageManager manager] getAssetIdentifier:tzImagePickerVc.selectedModels[index].asset] isEqualToString:[[TZImageManager manager] getAssetIdentifier:currentModel.asset]]) {
            [self select:_selectButton];
        }else{
            [tzImagePickerVc.selectedModels removeObjectAtIndex:index];
            if (self.photos) {
                [tzImagePickerVc.selectedAssets removeObject:_assetsTemp[_currentIndex]];
...
//        if ([_models[_currentIndex] isKindOfClass:[TZAssetModel class]]) {
...
//                    if (self.photos) {
//                        [_tzImagePickerVc.selectedAssets removeObjectAtIndex:_currentIndex];
...
//        }
        ...
        UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"确定要删除这张照片吗" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:@"删除" otherButtonTitles:nil];
...
- (void)deleteThisPhoto{
    ...
    _models = [NSMutableArray arrayWithArray:_tzImagePickerVc.selectedModels];
    if (_models.count > 0) {
        [_collectionView reloadData];
        [self refreshNaviBarAndBottomBarState];
    }else{
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (self.photoPreviewType != PhotoPreviewTypeOperation) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __block BOOL _weakIsOperation = _photoPreviewType == PhotoPreviewTypeOperation;
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    if (self.photoPreviewType != PhotoPreviewTypeOperation) {
        titleString = [NSString stringWithFormat:@"%zd/%zd", _currentIndex+1, _photos.count];//_photosTemp.count
    }else{
        titleString = [NSString stringWithFormat:@"%zd/%zd", _currentIndex+1, _models.count];
    }
    ...
    if (index < _tzImagePickerVc.maxImagesCount) {
