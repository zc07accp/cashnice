...
- (void)layoutSubviews {
    ...
    _selectedCountButton.frame = CGRectMake(self.tz_width - 24 - 30 - 10, 33, 24, 24);
}
...
- (UIImageView *)posterImageView {
    if (_arrowImageView == nil) {
        ...
        CGFloat designedScale = [ZAPP.zdevice getDesignScale:10];
        
        UIImageView *posterImageView = [[UIImageView alloc] init];
        posterImageView.contentMode = UIViewContentModeScaleAspectFill;
        posterImageView.clipsToBounds = YES;
        posterImageView.frame = CGRectMake(designedScale, designedScale, 7*designedScale, 7*designedScale);
...
- (UILabel *)titleLable {
    if (_titleLable == nil) {
        ...
        titleLable.frame = CGRectMake(90, 10, self.tz_width - 80 - 50, self.tz_height);
...
- (UIView *)sepLine{
    if (! _sepLine) {
        ...
        [sepLine mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self).with.mas_offset([ZAPP.zdevice getDesignScale:10]);
