...
- (instancetype)initWithMaxImagesCountCaptureCard:(NSInteger)maxImagesCount delegate:(id<TZImagePickerControllerDelegate>)delegate cardType:(NSInteger)cardType{
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index photoPreviewType:(PhotoPreviewType)photoPreviewType{
    TZPhotoPreviewController *previewVc = [[TZPhotoPreviewController alloc] init];
    previewVc.photoPreviewType = photoPreviewType;
...
//                if(photos.count && weakSelf.needCropFromCardID){
//                    UIImage *cropImage = [CameraEngine cropCapRegionIDCard:photos[0]];
//                    weakSelf.didFinishPickingPhotosHandle(@[cropImage],assets,isSelectOriginalPhoto);
//
//                }else{
                    weakSelf.didFinishPickingPhotosHandle(photos,assets,isSelectOriginalPhoto);
//                }
 
                _needCropFromCardID = NO;
            }
...
- (void)pushToPhotoPickerVc {
    ...
    if (_pushToPhotoPickerVc) {
        ...
        photoPickerVc.captureIDCardModel = self.captureIDCardModel;
        photoPickerVc.cardType = self.cardType;
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (id asset in selectedAssets) {
        ...
        if ([asset isKindOfClass:[UIImage class]]) {
            type = TZAssetModelMediaTypeImage;
        }
        TZAssetModel *model = [TZAssetModel modelWithAsset:asset type:type];
...
@interface TZAlbumPickerTableDelegate : NSObject

@end

@implementation TZAlbumPickerTableDelegate
...
- (void)setup {
    ...
    [self registerClass:[TZAlbumCell class] forCellReuseIdentifier:@"TZAlbumCell"];
}

- (void)setTarget:(UIViewController *)target
{
    _target = target;
    TZImagePickerController *imagePickerVc = (TZImagePickerController *)self.target.navigationController;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TZPhotoPickerController *photoPickerVc = (TZPhotoPickerController *)self.target;
    if ([photoPickerVc.model.name isEqualToString:model.name]) {
        cell.backgroundColor = ZCOLOR(COLOR_SEPERATOR_COLOR);
    }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //[self.target.navigationController pushViewController:photoPickerVc animated:YES];
    [tableView deselectRowAtIndexPath:indexPath animated:NO];
    
    [photoPickerVc showAlbumAction];
}
...
- (UIView *)titleView{
    ...
    [titleiew addSubview:titleLable];
