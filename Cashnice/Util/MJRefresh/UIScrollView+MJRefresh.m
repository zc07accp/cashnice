...
- (MJRefreshHeader *)addCNFreshWithRefreshingTarget:(id)target refreshingAction:(SEL)action dateKey:(NSString *)dateKey
{
    CNFreshHeader *header = [self addCNFreshHeader];
    header.refreshingTarget = target;
    header.refreshingAction = action;
    header.dateKey = dateKey;
    return header;
...
//    header.bottom = header.height;
//    
//    headerView.backgroundColor = ZCOLOR(COLOR_TAB_REFRESH_HEAD);
    self.header = header;
...
- (CNFreshHeader *)addCNFreshHeader
{
    CNFreshHeader *header = [[CNFreshHeader alloc] init];
