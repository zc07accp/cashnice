...
//        self.backgroundColor = [UIColor redColor];
        self.backgroundColor = ZCOLOR(COLOR_TAB_REFRESH_HEAD);
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    if (newSuperview) {
        self.mj_h = CGRectGetHeight([UIScreen mainScreen].bounds);/// MJRefreshHeaderHeight;
    }
}
...
//    CGFloat banner_height = scrollView.scrollView_banner_height ;
    CGFloat banner_height = 0;

    // 设置自己的位置
    self.mj_y = - self.mj_h - banner_height;
    ...
    if (self.updatedTimeHidden) { // 显示状态
        ...
        self.updatedTimeLabel.mj_y = self.mj_h - MJRefreshHeaderHeight;
        self.updatedTimeLabel.mj_h = MJRefreshHeaderHeight;
    } else { // 都显示
        CGFloat stateH = MJRefreshHeaderHeight * 0.55;
        CGFloat stateW = self.mj_w;
        // 1.状态标签
        _stateLabel.frame = CGRectMake(0,self.mj_h - MJRefreshFooterHeight- 10, stateW, stateH);
        
        // 2.时间标签
        CGFloat updatedTimeY = stateH + self.mj_h - MJRefreshFooterHeight - 10;
        CGFloat updatedTimeH = MJRefreshHeaderHeight - stateH;
...
- (void)adjustStateWithContentOffset
{
    ...
    if (self.state == MJRefreshHeaderStateRefreshing ) {
        if(_scrollView.contentOffset.y >= -_scrollViewOriginalInset.top ) {
            ...
            _scrollView.mj_insetT = MIN(_scrollViewOriginalInset.top + MJRefreshHeaderHeight,
...
//    NSLog(@"offsetY = %.2f， happenOffsetY=%.2f", offsetY, happenOffsetY);
...
//    UIScrollView *scrollView = (UIScrollView *)self.superview;
    CGFloat banner_height = 0;
//    scrollView.scrollView_banner_height ;
    
    // 普通 和 即将刷新 的临界点
    CGFloat normal2pullingOffsetY = happenOffsetY - MJRefreshHeaderHeight - banner_height;
    if (_scrollView.isDragging) {
        self.pullingPercent = (happenOffsetY - offsetY) / MJRefreshHeaderHeight;
...
-(void)restoreInitalStateComplete{
}
    ...
    switch (state) {
        case MJRefreshHeaderStateIdle: {
            if (oldState == MJRefreshHeaderStateRefreshing) {
                ...
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration delay:0.0 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                    // 修复top值不断累加
                    _scrollView.mj_insetT -= MJRefreshHeaderHeight;
                } completion:^(BOOL finished) {
                    [self restoreInitalStateDidComplete];
                }];
            }
            break;
        }
        ...
        case MJRefreshHeaderStateRefreshing: {
            [UIView animateWithDuration:MJRefreshFastAnimationDuration delay:0.0 options:UIViewAnimationOptionAllowUserInteraction|UIViewAnimationOptionBeginFromCurrentState animations:^{
                // 增加滚动区域
                CGFloat top = _scrollViewOriginalInset.top + MJRefreshHeaderHeight;
