...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
  ...
  @autoreleasepool {
    ...
    if (!CGRectIsEmpty(self.scanRect) && rotatedImage) {
        
        CGFloat width =  CGImageGetWidth(rotatedImage);
        CGFloat height = CGImageGetHeight(rotatedImage);
        
        CGRect cropRectd = CGRectMake(width*_scanRect.origin.x, height*_scanRect.origin.y, width*_scanRect.size.width, height*_scanRect.size.height);
        
        CGImageRef croppedImage = CGImageCreateWithImageInRect(rotatedImage, cropRectd);
...
int nameIndex = 1;
- (BOOL)saveImage:(CGImageRef)image{
    ...
    if (2 == nameIndex) {
        [self clearCacheWithFilePath:[paths objectAtIndex:0]];
    }
    
    NSString *fileName = [NSString stringWithFormat:@"%zd.png", nameIndex++];
    ...
    BOOL result = [UIImagePNGRepresentation(uiimage)writeToFile: filePath    atomically:YES];
    return result;
}
