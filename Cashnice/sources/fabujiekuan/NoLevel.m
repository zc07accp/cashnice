...
4、有多少好友向您多少额度的授信，您就可以发布多少额度的借款。(认证用户借款额度上限为%@，认证VIP用户上限为%@)\n", str1, str2];
...
[self setNavButton];
    [MobClick beginLogPageView:@"发布借款时要求身份验证"];
	[self setTitle:@"发布借款"];
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [MobClick endLogPageView:@"发布借款时要求身份验证"];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
	...
	if ([[segue destinationViewController] isKindOfClass:[NextButtonViewController class]]) {
        ...
        ((NextButtonViewController *)[segue destinationViewController]).titleString = @"身份验证";
	}
}
