...
- (instancetype)initWithFrameOrigin:(CGPoint)origin width:(CGFloat)width {
    ...
    CGFloat weekLineHight = 0.75 * (width / 7.0);
    CGFloat monthHeight = 6 * weekLineHight;
    
    // 星期头部栏高度
    CGFloat weekHeaderHeight = 1.0 * weekLineHight;
    ...
    if (self = [super initWithFrame:CGRectMake(origin.x, origin.y, width, _calendarHeight)]) {
        ...
        _calendarScrollView = [self setupCalendarScrollViewWithFrame:CGRectMake(0.0, calendarHeaderHeight + 0, width, monthHeight+weekLineHight)];
...
- (void)setMonthHightPoint:(NSArray *)date{
    [_calendarScrollView setMonthHightPoint:date];
}
...
- (UIButton *)setupCalendarHeaderButtonWithFrame:(CGRect)frame {
    ...
    button.titleLabel.font = CNFont_26px;
    ...
    line.backgroundColor = CN_COLOR_DD_GRAY;
...
- (UIView *)setupWeekHeadViewWithFrame:(CGRect)frame {
    ...
    for (int i = 0; i < weekArray.count; ++i) {
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(i * width, 4.0, width, height)];
...
- (void)setDidDisSelectDayHandler:(DidSelectDayHandler)didDisSelectDayHandler{
    _didDisSelectDayHandler = didDisSelectDayHandler;
    if (_didDisSelectDayHandler != nil) {
        _calendarScrollView.didDisSelectDayHandler = _didDisSelectDayHandler;
    }
}

- (void)setDidSelectMonthHandler:(DidSelectMonthHandler)didSelectMonthHandler{
    _didSelectMonthHandler = didSelectMonthHandler;
    if (_didSelectMonthHandler != nil) {
        _calendarScrollView.didSelectMonthHandler = _didSelectMonthHandler;
    }
}
